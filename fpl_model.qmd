...
model_df <- player_df |>
  ...
  select(element,fixture, round,playername, total_points, position, team_name, opp_team_name, was_home,  starts_with("prev"), starts_with("average")) 
...
```{r}
xgb_recipe <- recipe(total_points ~ position + team_name + opp_team_name + was_home + prev_ict +prev_value + prev_starts+ prev_points_1 + prev_minutes_1 + prev_points_2 + prev_minutes_2 + prev_minutes_3 + prev_minutes_4 + prev_points_3 + prev_points_4 + prev_points_5 + average_points5 + average_minutes5, data = df_train) |> 
...
df_test$pred <- pred$.pred


metrics(df_test, truth= total_points, estimate = pred)
```
```{r}
